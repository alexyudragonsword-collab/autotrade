<template>
  <el-card>
    <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 12px; flex-wrap: wrap">
      <el-select v-model="broker" placeholder="选择账户" style="width: 180px">
        <el-option v-for="a in brokerAccounts" :key="a.name" :label="`${a.name}（${a.type}）`" :value="a.name" />
      </el-select>
      <el-input v-model="underlying" placeholder="标的：US.AAPL / HK.00700" style="width: 220px"
                @keyup.enter="loadExpirations" />
      <el-button type="primary" :loading="loadingExp" @click="loadExpirations">加载到期日</el-button>
      <el-select v-if="expirations.length" v-model="expiry" placeholder="到期日" style="width: 160px"
                 @change="loadChain">
        <el-option v-for="e in expirations" :key="e" :label="fmtDate(e)" :value="e" />
      </el-select>
      <el-switch v-if="expiry" v-model="withQuotes" active-text="含报价" @change="loadChain" />
      <span v-if="chain" style="color: #6b7280; font-size: 13px; margin-left: auto">
        正股 {{ chain.underlying_price != null ? chain.underlying_price : '-' }}　乘数 ×{{ chain.multiplier }}
      </span>
    </div>
    ...
    <el-table v-if="chain" :data="chain.rows" v-loading="loadingChain" size="small"
      ...
      <el-table-column label="Call" align="center">
        <el-table-column label="买价" width="80" align="right">
          <template #default="{ row }">{{ n(row.call?.bid) }}</template>
        </el-table-column>
        <el-table-column label="卖价" width="80" align="right">
          <template #default="{ row }">{{ n(row.call?.ask) }}</template>
        </el-table-column>
        <el-table-column label="最新" width="80" align="right">
          <template #default="{ row }">{{ n(row.call?.last) }}</template>
        </el-table-column>
        <el-table-column label="" width="80" align="center">
          <template #default="{ row }">
            <el-button v-if="row.call" link type="danger" size="small" @click="trade(row.call)">交易</el-button>
          </template>
        </el-table-column>
      </el-table-column>
      <el-table-column prop="strike" label="行权价" width="100" align="center">
        <template #default="{ row }">
      ...
      <el-table-column label="Put" align="center">
        <el-table-column label="" width="80" align="center">
          <template #default="{ row }">
            <el-button v-if="row.put" link type="success" size="small" @click="trade(row.put)">交易</el-button>
          </template>
        </el-table-column>
        <el-table-column label="买价" width="80" align="right">
    ...
    <el-empty v-else-if="!error" description="选择账户与标的后加载期权链（需要 IBKR 或富途账户在线）" />

    <el-alert type="info" :closable="false" show-icon style="margin-top: 12px"
              title="点击「交易」预填下单框；卖出无持仓即为卖方开仓，默认档要求备兑（Call）或现金担保（Put），裸卖需在风控设置显式开启。期权交易需在风控设置中先启用。" />
  </el-card>
...
async function loadExpirations() {
  ...
  try {
    ...
    if (!data.expirations.length) error.value = '该标的没有可交易期权'
  } catch (e) {
