<template>
  <div v-loading="loading">
    <el-row :gutter="16">
      <el-col :span="6">
        <el-card>
          <div class="stat-label">{{ $t('交易总开关') }}</div>
          <div style="display: flex; align-items: center; gap: 12px; margin-top: 8px">
            <el-switch v-model="tradingEnabled" size="large" :active-text="$t('允许交易')"
                       :inactive-text="$t('已停止')" @change="toggleKillSwitch" />
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card><div class="stat-label">{{ $t('今日信号') }}</div><div class="stat-num">{{ summary.signals_today }}</div></el-card>
      </el-col>
      <el-col :span="6">
        <el-card><div class="stat-label">{{ $t('今日订单') }}</div><div class="stat-num">{{ summary.orders_today }}</div></el-card>
      </el-col>
      <el-col :span="6">
        <el-card><div class="stat-label">{{ $t('今日风控拦截') }}</div>
        ...
        <el-card :header="$t('券商连接状态')">
          <div v-for="(info, name) in summary.brokers" :key="name" class="broker-row">
            ...
            <span v-if="summary.accounts?.[name]" style="font-size: 13px">
              {{ $t('净值') }} {{ fmt(summary.accounts[name].net_value) }}
            </span>
          </div>
        </el-card>
        <el-card style="margin-top: 16px">
          <template #header>{{ $t('自选关注') }} ({{ watchlist.length }})</template>
          <el-table :data="watchlist" size="small" max-height="220" v-if="watchlist.length">
            <el-table-column prop="symbol" :label="$t('代码')" />
            <el-table-column prop="last_close" :label="$t('最近收盘')"><template #default="{ row }">{{ fmt(row.last_close) }}</template></el-table-column>
            <el-table-column label="" width="60">
              <template #default="{ row }">
                <el-button link type="danger" size="small" @click="removeWatch(row.symbol)">{{ $t('移除') }}</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-empty v-else :description="$t('选股器结果可一键加自选')" :image-size="48" />
        </el-card>
      </el-col>
      <el-col :span="16">
        <el-card :header="$t('最近信号')">
          <el-table :data="summary.recent_signals" size="small">
            ...
            <el-table-column prop="strategy_name" :label="$t('策略')" width="110" />
    ...
    <el-card :header="$t('最近订单')" style="margin-top: 16px">
      <el-table :data="summary.recent_orders" size="small">
        ...
        <el-table-column prop="avg_fill_price" :label="$t('成交均价')" width="100" />
        <el-table-column prop="status" :label="$t('状态')" width="120"><template #default="{ row }"><order-status :status="row.status" /></template></el-table-column>
...
async function toggleKillSwitch(value) {
  if (!value) {
    try {
      await ElMessageBox.confirm(tr('确定要紧急停止全部交易吗？新信号将被风控直接拒绝。'), 'Kill Switch', { type: 'warning' })
    } catch {
