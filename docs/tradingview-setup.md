...
告警 JSON 追加三个字段即可把正股信号映射到期权合约下单（三者必须同时提供）：

```json
{"...": "...同上...",
 "expiry": "2025-09-19",   // 或 20250919
 "strike": 230,
 "right": "C"}             // C/P/call/put
```

系统会组合成内部合约符号（如 `US.AAPL|20250919|C|230`）。注意：
- 期权交易需先在风控设置中启用；卖出无持仓即为卖方开仓，受备兑/现金担保（默认档）或裸卖名义限额约束
- 数量单位是**张**（每张乘数见期权链页面，美股期权 100）
