...
| 🎡 期权内置策略 | **CoveredCall 备兑开仓**（滚动卖虚值 Call 收权利金，临近到期自动买回滚动）、**WheelStrategy 车轮**（现金担保卖 Put ↔ 被行权接货后自动切换备兑 Call）：按 cron 自动运行，实时查链按虚值比例/到期区间选合约，走统一风控管道；paper 执行 + 真实券商链报价可安全模拟验证 |
